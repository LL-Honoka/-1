...
class PMI:
    ...
    def get_pmi(self):
        ...
        dict_seed_good = {'香':1,'好吃':1,'美味':1,'准时':1,'棒':1}
        dict_seed_bad = {'吐':1,'难吃':1,'反胃':1,'超时':1,'恶心':1}
        for word1 in dict_frq_word:
            wordpercent1 = dict_frq_word[word1]
            dict_pmi_total[word1] = 0 
            for word2 in dict_seed_good:
                if word1 == word2:
                    continue
                wordpercent2 = dict_seed_good[word2]
                list_together=[]
                ...
                if together_probability > self.minitogether:
                    dict_pmi_total[word1] += self.calculate_nmi(together_probability, wordpercent1, wordpercent2)
            for word3 in dict_seed_bad:
                if word1 == word3:
                    continue
                wordpercent3 = dict_seed_bad[word3]
                list_together=[]
                list_together.append(word1)
                list_together.append(word3)
                together_probability = self.calcularprobability(self.document, list_together)
                if together_probability > self.minitogether:
                    dict_pmi_total[word1] -= self.calculate_nmi(together_probability, wordpercent1, wordpercent3)
        return dict_pmi_total
